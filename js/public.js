...
function pageNameToType(pageName) {
  const map = {
    'news': 'News',
    'feature': 'Feature',
    'opini': 'Opini',
...
async function renderCategoryPage(pageName, container) {
  if (allArticles.length === 0) await loadNewsData();

  const targetType = pageNameToType(pageName);

  const filtered = allArticles.filter(a => 
    a.type === targetType && a.status === 'published'
  );

  const displayName = capitalizeFirst(pageName.replace('-', ' '));

  container.innerHTML = `
    <h2 class="font-serif text-3xl font-bold text-slate-800 mb-2 capitalize">${displayName}</h2>
